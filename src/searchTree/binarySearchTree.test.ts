...
import { STNode, BinarySearchTree as SearchTree } from "./binarySearchTree.js";
import {
...
const expectedNodes: STNode<string, number | number[]> = {
  key: `b`,
  value: 2,
  left: { key: `a`, value: 1, left: null, right: null },
  right: { key: `c`, value: 3, left: null, right: null },
};
...
describe(`SearchTree`, () => {
  ...
  test(`stores key/value pairs correctly with a match handler (additive)`, async () => {
    ...
    expect(root).toEqual({
      ...expectedNodes,
      right: { key: `c`, value: 7, left: null, right: null },
    });
  });
  ...
  test(`stores key/value pairs correctly with a match handler (replace)`, async () => {
    ...
    expect(root).toEqual({
      ...expectedNodes,
      right: { key: `c`, value: 4, left: null, right: null },
    });
  });
  ...
  test(`stores key/value pairs correctly with a match handler (array)`, async () => {
    ...
    expect(root).toEqual({
      ...expectedNodes,
      right: { key: `c`, value: [3, 4], left: null, right: null },
    });
  });
});

...
import { ColumnOrder, Data, DataRow, FullTree, TreeRoot, TreeElement, TreeLeaves, TreeBranch } from './tree.types.js';
...
const convertArrayToTree = ({ data, columnOrder }: { data: Data, columnOrder: ColumnOrder }): FullTree => {
    ...
    rows.forEach((currentRow: DataRow) => {
        ...
        columnOrder.forEach((columnIndex, columnOrderIndex) => {
             ...
             */
                ...
                (currentElement as TreeLeaves).push(columnValue);
        });
    });
...
const readAndConvert = async (fileName: string, columnOrder: ColumnOrder): Promise<FullTree> => {
